...
function EntityDeclump.try_declump(other, self, dt)
    ...
    if dist_squared < (rs * rs) then
        ...
        -- If other is passive, apply force to self instead (push self away)
        if other.passive_declump then
            local other_force = other.declump_force * self.self_declump_modifier
...
function EntityDeclump:do_declump(dt)
    -- Passive objects stay in the grid but don't actively query
    if self.passive_declump then return end
