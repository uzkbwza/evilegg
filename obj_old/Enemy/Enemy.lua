...
function Enemy:new(x, y)
    ...
    Enemy:implement(Mixins.Fx.RetroRumble)
    Enemy:implement(Mixins.Behavior.Flippable)
    Enemy:implement(Mixins.Behavior.FreezeFrames)
    Enemy:implement(Mixins.Behavior.BumpCollision)
    Enemy:implement(Mixins.Behavior.Hittable)
    Enemy:implement(Mixins.Behavior.GridTerrainQuery)
...
function Enemy:draw()
    if self:is_timer_running("hit_flash") and self.tick % 2 == 0 then
        return
    end
