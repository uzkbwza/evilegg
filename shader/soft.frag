...
uniform float glow_blur_size = 0.08;         // Controls the glow blur size
uniform int glow_blur_samples = 7;           // Controls the number of samples in the glow blur
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    // Apply glow effect with dedicated parameters
    vec4 glow_blurred = vec4(0.0);
    float glow_total_weight = 0.0;
    
    int glow_half_samples = glow_blur_samples / 2;
        for (int i = -glow_half_samples; i <= glow_half_samples; i++) {
            for (int j = -glow_half_samples; j <= glow_half_samples; j++) {
                vec2 offset = vec2(float(i), float(j)) * glow_blur_size * pixel_size;
                float weight = exp(-0.5 * (float(i * i + j * j) / float(glow_half_samples * glow_half_samples)));
                weight = pow(weight, glow_curve);
                glow_blurred += Texel(texture, uv + offset) * weight;
                glow_total_weight += weight;
            }
        }
        glow_blurred /= glow_total_weight;
        
        // Blend glow with base color
        float glow_r = max(glow_blurred.r, base.r) + (glow_blurred.r * glow_boost);
        float glow_g = max(glow_blurred.g, base.g) + (glow_blurred.g * glow_boost);
        float glow_b = max(glow_blurred.b, base.b) + (glow_blurred.b * glow_boost);
        
        vec4 pixel = vec4(mix(base.rgb, vec3(glow_r, glow_g, glow_b), intensity), base.a);
        pixel.a = 1.0;
        return pixel;
